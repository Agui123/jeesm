...
function checkOrderForm(frm)
{
  ...
  if (Utils.isEmpty(frm.elements['display_info'].value))
  {
    alert('请先保存收货地址！');
    return false;
  }
	
  if (!document.getElementById("flow_consignee")){
	  
	  alert(flow_no_address);
	  return false;
  }
...
$(document).ready(function(){
	...
	$(".consignee_form").css("display", "block");
	$(".consignee_info").css("display", "none");
	})
	/*修改收货地址切换 end*/
    
	/*购物登录tab begin*/
	$(".logit h2").click(function(){
	$(this).addClass("cur").siblings().removeClass("cur");
	index = $(this).index();
	$(".login_info").css("display","none");
	$(".login_info").eq(index).css("display","block");
	})
	/*购物登录tab end*/
})
